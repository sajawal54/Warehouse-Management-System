...
const SalesOrderForm = ({ onClose }) => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.customer_ref.trim()) newErrors.customer_ref = 'Customer reference is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = error.response?.data?.detail || 'Failed to create sales order';
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-2 rounded-lg border ${
              errors.customer_ref ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
            } dark:bg-gray-700 dark:text-white`}
            placeholder="Customer name or order #"
          />
          {errors.customer_ref && <p className="mt-1 text-sm text-red-500">{errors.customer_ref}</p>}
        </div>
