...
def create_purchase_order(purchase : PurchaseOrderCreate , db : Session = Depends(get_db)):
...
def receive_purchase_order(purchase_order_id: int, receive_data: ReceivePurchaseOrderRequest, db: Session = Depends(get_db)):
    ...
    if order.status in ["Received", "Closed"]:
        raise HTTPException(status_code=400, detail="Order is already received or closed")

    complete_received = True

    for item in receive_data.items:
        order_item = db.query(PurchaseOrderItem).filter(PurchaseOrderItem.purchase_order_id == purchase_order_id,PurchaseOrderItem.product_id == item.product_id).first()

        if not order_item:
            raise HTTPException(status_code=400, detail=f"Product {item.product_id} not in this order")

        new_total_received = order_item.received_qty + item.receive_qty
        if new_total_received > order_item.ordered_qty:
            raise HTTPException(status_code=400, detail=f"Received quantity exceeds ordered quantity for product {item.product_id}")
...
def get_order_by_id(purchase_order_id : int , db : Session = Depends(get_db)):
  order = db.query(PurchaseOrder).filter(PurchaseOrder.id == purchase_order_id).first()
...
def submit_purchase_order(purchase_order_id : int , db : Session = Depends(get_db)):
  order = db.query(PurchaseOrder).filter(PurchaseOrder.id == purchase_order_id).first()
  if not order:
    raise HTTPException(status_code=404 , detail="Purchase order not found")
